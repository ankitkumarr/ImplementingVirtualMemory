...
void memoryMaxer();
int evict_from_ram();
int findentry(int index, int mem);
int evict_from_ssd();
int evict_from_hdd();
...
void memoryMaxer() {
	...
	/**for(i = 0; i < 128; i++) {
		printf("Value at ptable index %i is %d\n", i, *(get_value(indexes[i])));
	}**/
...
int evict_from_ram() {
	...
	int index = page_eviction_algorithm2(1);
	int i = 0;
	int nindex;
	int flag = 0;
	for(i = 0; i < 100; i++) {
	...
	iind = findentry(index, 1);

	//changing the values of the page entry that was transferred
	ptable[iind].memflag = 2;
	ptable[iind].index = nindex;
	ram[index] = -1;
	return index;
}
...
int evict_from_ssd() {
	...
	int index = page_eviction_algorithm2(2);
	int i = 0;
	int nindex;
	int flag = 0;
	for (i = 0; i < 1000; i++) {
...
int evict_from_hdd() {
	...
	int index = page_eviction_algorithm2(3);
...
//least accessed by user algorithm

int page_eviction_algorithm2 (int memflag) {

	printf("Removing a page which is least accessed. If more than one, select anyone\n");
	...
	if ( memflag == 1) {
		...
		for (i = 0; i < 25; i++) {
			temp = findentry(i,1);
			if ((ptable[temp].modified < mod) && (ptable[temp].valid == 1)) {

				mod = ptable[temp].modified;
				ind = i ;
			}
		}

		printf("Turns out, the page to be removed is %d\n", findentry(ind, 1));
