...
int evict_from_ram() {
	...
	int index = page_eviction_algorithm3(1);
	int i = 0;
	int nindex;
	int flag = 0;
	for(i = 0; i < 100; i++) {
...
int evict_from_ssd() {
	...
	int index = page_eviction_algorithm3(2);
	int i = 0;
	int nindex;
	int flag = 0;
	for (i = 0; i < 1000; i++) {
...
int evict_from_hdd() {
	...
	int index = page_eviction_algorithm3(3);
...
int page_eviction_algorithm3 (int memflag) {
	...
	if(memflag == 1) {
		...
		for ( i = 0; i < 25; i++) {
			temp = findentry(i, 1);
			if((temp->newflag > mod) && (temp->valid == 1)) {
				mod = temp->newflag;
				ind = i;
			}
		}
	...
	printf("The page that will be evicted decided by FIFO logic is at pagetable index %d\n", ind);
	return ind;
